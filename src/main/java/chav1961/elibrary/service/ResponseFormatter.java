...
public class ResponseFormatter {
	...
	public static final String	SNIPPET_CONTENT_LABEL = "html.snippet.content.label";
	...
	public static enum ContentPath {
		...
		SNIPPET_INNER_ITEM("/content/getinneritem"),
	...
	public static String buildItemDescriptor(final Localizer localizer, final BookDescriptor desc, final Connection conn) {
		...
		sb.append("<p>").append(getSeries(localizer, desc, conn)).append(" ").append(getAuthors(localizer, desc, conn)).append("</p>");
		sb.append("<p>").append(getPublisher(localizer, desc, conn)).append(", ").append(getYear(localizer, desc)).append("</p>");
		sb.append(getContent(localizer, desc, conn));
	...
	public static String buildSubItemDescriptor(final Localizer localizer, final InnerBookDescriptor desc, final Connection conn) {
		final StringBuilder	sb = new StringBuilder();
		
		sb.append("<div class=\"").append(SNIPPET_DIV_CLASS).append("\">\n");
		sb.append("<table class=\"").append(SNIPPET_TABLE_CLASS).append("\">\n");
		sb.append("<tr><td><img src=\"").append(buildHRef(ContentPath.SNIPPET_IMAGE,desc.id)).append("\" alt=\"").append("\" width=200").append(" height=297").append("></td>");
		sb.append("<td><h3>").append(desc.title).append("</h3>\n");
		sb.append("<p>").append(getAuthors(localizer, desc, conn)).append("</p>");
		sb.append("<p>").append(getPublishedIn(localizer, desc, conn)).append(" ").append(localizer.getValue(SNIPPET_PAGE_LABEL)).append(" ").append(desc.page).append("</p>");
		sb.append("<hr/>\n");
		sb.append(getTags(localizer, desc));
		sb.append("</td></tr></table>\n");
		sb.append("</div>\n");
	...
	private static String getAuthors(final Localizer localizer, final InnerBookDescriptor desc, final Connection conn) {
		final StringBuilder	sb = new StringBuilder();
	...
	private static String getPublishedIn(final Localizer localizer, final InnerBookDescriptor desc, final Connection conn) {
		final StringBuilder	sb = new StringBuilder(); 
		
		try(final PreparedStatement	ps = conn.prepareStatement("select \"bl_Id\",\"bl_Title\" from \"elibrary\".\"booklist\" where \"bl_Id\" = ?")) {
			ps.setLong(1, desc.parentRef);

			try(final ResultSet		rs = ps.executeQuery()) {
				if (rs.next()) {
					sb.append(localizer.getValue(SNIPPET_PUBLISHED_IN_LABEL)).append(": <a href=\"").append(buildHRef(ContentPath.SNIPPET_ITEM, rs.getLong("bl_Id"))).append("\">").append(escapeHtmlString(rs.getString("bl_Title"))).append("</a></li>");
				}
			}
			return sb.toString();
		} catch (SQLException e) {
	...
	private static String getTags(final Localizer localizer, final InnerBookDescriptor desc) {
		if (desc.tags != null && desc.tags.length > 0) {
			final StringBuilder	sb = new StringBuilder();
	...
	private static String getContent(final Localizer localizer, final BookDescriptor desc, final Connection conn) {
		final StringBuilder	sb = new StringBuilder();
		
		try(final PreparedStatement	ps = conn.prepareStatement("select \"bl_Id\", \"bl_Page\", \"bl_Title\" from \"elibrary\".\"booklist\" where \"bl_Parent\" = ? order by \"bl_Page\"")) {
			boolean theSameFirst = true;
			ps.setLong(1, desc.id);
			...
			try(final ResultSet		rs = ps.executeQuery()) {
				while (rs.next()) {
					if (theSameFirst) {
						theSameFirst = false;
						sb.append("<h4>").append(localizer.getValue(SNIPPET_CONTENT_LABEL)).append("</h4>");
						sb.append("<ul>");
					}
					sb.append("<li>").append(localizer.getValue(SNIPPET_PAGE_LABEL)).append(" ").append(rs.getInt("bl_Page")).append(": <a href=\"").append(buildHRef(ContentPath.SNIPPET_INNER_ITEM, rs.getLong("bl_Id"))).append("\">").append(escapeHtmlString(rs.getString("bl_Title"))).append("</a></li>");
				}
			}
			if (!theSameFirst) {
				sb.append("</ul>");
